...
//! `src/lukeworks.rs` into the highlighted `CODE_BACKDROP` the hero sits on.
//!
...
fn main() {
    ...
    write_backdrop(Path::new(&out_dir), &options, &plugins);
...
/// itself, run through the syntect adapter the blog's fenced blocks use. The
/// scope classes it emits already have light and dark rules in
/// `style/tailwind.css`, so the hero needs no image asset.
///
...
/// bundle, blur or no blur. Today's window is a route table and a stylesheet
/// component, all of it already reachable from the site's own URLs. Point this
/// at a file chosen for the backdrop if `src/lukeworks.rs` ever grows something
/// that should not be read off the homepage, such as an unlisted route.
fn write_backdrop(out_dir: &Path, options: &Options, plugins: &Plugins<'_>) {
    ...
    // A tilde fence: a doc comment in the source may carry a ``` of its own, and
    // that would close a backtick fence early. What closes a tilde fence is a
    // line of tildes, and comrak renders the spill as markdown with raw HTML
    // enabled, so a `view!` body would reach the homepage as live markup.
    assert!(
        !head.iter().any(|line| line.trim_start().starts_with("~~~")),
        "{}: a ~~~ in the first {BACKDROP_LINES} lines closes the backdrop's fence",
        path.display()
    );
    ...
    let mut out = String::from(
        "// @generated by build.rs from src/lukeworks.rs — do not edit.\n\n\
         /// The site's own source, highlighted, for the homepage hero to sit on.\n\
         ///\n\
